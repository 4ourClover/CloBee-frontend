...
export const fetchAllCards = async (cardType: string, page: number, size: number): Promise<CardPageDTO> => {
    try {
        ...
        const response = await axios.get<CardPageDTO>("http://localhost:8080/server/card/getCardList", {
...
export const fetchCardDetail = async (cardInfoId: number): Promise<CardBenefitDetail[]> => {
    const response = await axios.get<CardBenefitDetail[]>("http://localhost:8080/server/card/getCardDetail", {
        params: { cardInfoId },
    })
    return response.data
}
...
export const applyCard = async (cardInfoId: number, cardBrand: number): Promise<string> => {
    try {
        const response = await axios.get<string>("http://localhost:8080/server/card/apply", {
...
export const fetchMyCards = async (userId: number): Promise<UserCardListDTO[]> => {
    const response = await axios.get<UserCardListDTO[]>("http://localhost:8080/server/card/getMyCardList", {
        params: { userId },
    })
    return response.data
}
...
export const fetchCardPerformance = async (
    ...
    try {
        ...
        const response = await axios.get<UserCardPerformanceDetail>("http://localhost:8080/server/card/getPerformance", { params })
...
export const searchCards = async (cardName: string): Promise<CardListDTO[]> => {
    try {
        ...
        const response = await axios.get<CardListDTO[]>("http://localhost:8080/server/card/search", {
            params: { cardName },
        })
...
export const addUserCard = async (userId: number, cardInfoId: number, userCardType: number): Promise<string> => {
    try {
        const response = await axios.post<string>("http://localhost:8080/server/card/addCard", {
            userId,
            cardInfoId,
            userCardType,
        })
...
export const deleteUserCard = async (userId: number, cardInfoId: number): Promise<string> => {
    const response = await axios.delete<string>("http://localhost:8080/server/card/delCard", {

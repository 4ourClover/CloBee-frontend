...
export default function ForgotPasswordPage() {
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        if (!email) {
            toast({
                title: "이메일을 입력해주세요",
                description: "임시 비밀번호를 받을 이메일 주소가 필요합니다.",
                variant: "destructive",
            })
            return
        }
                ...
                {!isSubmitted ? (
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="space-y-2">
                            <Label htmlFor="email" className="text-[#5A3D2B]">이메일</Label>
                            <div className="relative">
                                <Input
                                    ...
                                    required
                                />
                                <Mail className="absolute left-3 top-3 h-4 w-4 text-[#00A949]" />
                            </div>
                        </div>
                        <Button
                            type="submit"
                    ...
                    <div className="space-y-6">
                        <div className="bg-[#F5FAF0] rounded-lg p-6 text-center">
                            <div className="w-16 h-16 bg-[#75CB3B]/20 rounded-full flex items-center justify-center mx-auto mb-4">
                                <Mail className="h-8 w-8 text-[#00A949]" />
                            </div>
                            <h2 className="text-lg font-bold text-[#5A3D2B] mb-2">이메일 발송 완료</h2>
                            <p className="text-sm text-[#5A3D2B]/80 mb-1">{email}로 임시 비밀번호가 발송되었습니다.</p>
                            <p className="text-xs text-[#5A3D2B]/60">이메일이 도착하지 않았다면 스팸함을 확인해주세요.</p>
                        </div>
